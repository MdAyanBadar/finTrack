...
export const ingestMessage = async (req, res) => {
  ...
  if (!key?.startsWith("ft_")) return res.status(401).json({ message: "Missing import key" });

  const user = await prisma.user.findUnique({
    where: { ingestKeyHash: sha256(key) },
    select: { id: true },
  });
  if (!user) return res.status(401).json({ message: "Invalid import key" });
